...
LRATE = 0.009/SIZE_TRAIN;
W_1= -0.18 + 0.184*2*rand(27,55);
...
while true
    ...
    delta_p2 = (ones(536,55)-OP2s .*OP2s) .*(delta_pop*(W_2'));
...
TEST_T=[ones(size(AEP_TEST,1),1) ; zeros(size(NOAEP_TEST,1),1) ];
OP2s=tanh(TEST*W_1);
OP3s=tanh(OP2s*W_2);
for i=1:SIZE_TEST
   if TEST_T(i)==1 && OP3s(i)>0
        test_tp=test_tp+1;
   end
